...
model.add(Conv2D(128, 3, 3, border_mode='same', activation='relu', kernel_initializer='he_normal'))
model.add(Conv2D(128, 3, 3, border_mode='same', activation='relu', kernel_initializer='he_normal'))
model.add(MaxPooling2D(pool_size=(2, 2)))

model.add(Conv2D(256, 3, 3, border_mode='same', activation='relu', kernel_initializer='he_normal'))
model.add(Conv2D(256, 3, 3, border_mode='same', activation='relu', kernel_initializer='he_normal'))
model.add(MaxPooling2D(pool_size=(2, 2)))
...
def process_file(picPath):
    ...
    for i in range(len(file_list)):
        ...
        if file_list[i].endswith('.jpg'):
            ...
            img = img_ / 255.0
            X_list.append(img)
            B = re.findall(r'-?\d+\.\d+', file_list[i])
            Y_list.append(B)
...
for j in range(len(fileList)):
    ...
    if fileList[j].endswith('.jpg'):
        ...
        B_test = re.findall(r'-?\d+\.\d+', fileList[j])
        ...
        with open('test_result.csv', 'a', newline='') as myFile:
            ...
            myWriter.writerow([num_valid, fileList[j], B_test[0], B_test[1], predict_B[0][0], predict_B[0][1], 
                               predict_B_best[0][0], predict_B_best[0][1]])
...
for k in range(len(fileList_)):
    ...
    if fileList_[k].endswith('.jpg'):
        ...
        B_train = re.findall(r'-?\d+\.\d+', fileList_[k])
        ...
        with open('train_result.csv', 'a', newline='') as myFile_:
            ...
            myWriter_.writerow([num_valid, fileList[k], B_train[0], B_train[1], predict_B[0][0], predict_B[0][1], 
                                predict_B_best[0][0], predict_B_best[0][1]])
