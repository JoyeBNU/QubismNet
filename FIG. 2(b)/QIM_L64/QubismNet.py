...
plot_model(model, to_file='model.pdf', show_shapes=True)  # 画出模型结构图，并保存成图片
...
with open('2/test_result.txt', 'a') as fre:
...
for j in range(len(fileList)):
    ...
    if fileList[j].endswith('.jpg'):
        ...
        with open('2/test_result.csv', 'a', newline='') as myFile:
...
for k in range(len(fileList_)):
    ...
    if fileList_[k].endswith('.jpg'):
        ...
        with open('2/train_result.csv', 'a', newline='') as myFile_:
