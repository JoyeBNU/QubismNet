...
from BasicFunctions import combination
import copy
...
def position_breathing_kagome2n(length, bound_cond):
    '''
    length : para['num_columns'], kagome lattice共有几列
    bound_cond : 边界条件的类型
    '''
    if bound_cond.lower() in ['pbc', 'periodic']:
        pos1 = [[0, 4], [4, 5], [0, 5]]
        ...
        raise ValueError("bound_cond must pbc")
    pos1_ans = copy.deepcopy(pos1)
    pos2_ans = copy.deepcopy(pos2)
    for n in range(length-1):
        for m in range(len(pos1)):
            pos1[m][0] = pos1[m][0] + 6
            pos1[m][1] = pos1[m][1] + 6
        for k in range(len(pos2)):
            pos2[k][0] = pos2[k][0] + 6
            pos2[k][1] = pos2[k][1] + 6
        pos1_ans = pos1_ans + copy.deepcopy(pos1)
        pos2_ans = pos2_ans + copy.deepcopy(pos2)
    # 周期结构需要修改最后的几个连接
    pos2_ans[-1][1] = pos2_ans[-1][0]
    pos2_ans[-1][0] = 1
    pos2_ans[-2][1] = pos2_ans[-2][0]
    pos2_ans[-2][0] = 1
    pos2_ans[-4][1] = pos2_ans[-4][0]
    pos2_ans[-4][0] = 0
    pos2_ans[-5][1] = pos2_ans[-5][0]
    pos2_ans[-5][0] = 0
    return pos1_ans+pos2_ans
