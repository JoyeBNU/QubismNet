...
from BasicFunctions import print_error, print_options, print_dict, \
    project_path
...
def parameter_dmrg_kagome2n():
    para = dict()
    para['spin'] = 'half'
    para['num_columns'] = 2  # Depth of three branches
    # in chain, the interactions are assumed to be uniform; if not, use parameter_dmrg_arbitrary instead
    para['jxy'] = 0
    para['jz'] = 1
    para['hx'] = 0.5
...
def make_consistent_parameter_dmrg(para):
    if para['lattice'] == 'chain':
    ...
    elif para['lattice'] == 'jigsaw':
        if para['bound_cond'] == 'open':
            if para['l'] % 2 == 0:
        ...
        para['coeff1'] = np.ones((para['l'], 1))
        para['coeff2'] = np.zeros((para['positions_h2'].shape[0] * 3, 1))
        for n in range(0, para['l'] - (para['bound_cond'] == 'open')):
            para['coeff2'][n * 3] = para['jxy'] / 2
            para['coeff2'][n * 3 + 1] = para['jxy'] / 2
            para['coeff2'][n * 3 + 2] = para['jz']
        for n in range(para['l'] - (para['bound_cond'] == 'open'), para['positions_h2'].shape[0]):
        ...
        para['positions_h2'] = hm.position_kagome2n(para['num_columns'], para['bound_cond'])
        para['positions_h2'] = np.array(para['positions_h2']).astype(int)
        para['l'] = para['positions_h2'].max() + 1
        para['index1'] = np.mat(np.arange(0, para['l']))
        para['index1'] = np.vstack((para['index1'], 6 * np.ones((1, para['l'])))).T.astype(int)
        para['index2'] = hm.interactions_position2full_index_heisenberg_two_body(para['positions_h2'])
        ...
        for n in range(0, para['positions_h2'].shape[0]):
            ...
            para['coeff2'][n * 3 + 1] = para['jxy'] / 2
            para['coeff2'][n * 3 + 2] = para['jz']
        para['data_exp'] = 'kagome2nColumns%d_j(%g,%g)_h(%g,%g)_chi%d' % \
                           (para['num_columns'], para['jxy'], para['jz'], para['hx'],
                            para['hz'], para['chi'])
...
def make_consistent_parameter_dmrg_2(para):
    if para['lattice'] == 'kagome2n':
        para['positions_h2'] = hm.position_breathing_kagome2n(para['num_columns'], para['bound_cond'])
        para['positions_h2'] = np.array(para['positions_h2']).astype(int)
        para['l'] = para['positions_h2'].max() + 1
        para['index1'] = np.mat(np.arange(0, para['l']))
        para['index1'] = np.vstack((para['index1'], 6 * np.ones((1, para['l'])))).T.astype(int)
        ...
        para['coeff1'] = np.ones((para['l'], 1))
        para['coeff2'] = np.zeros((para['positions_h2'].shape[0] * 3, 1))
        for n in range(0, int(para['positions_h2'].shape[0]/2)):
            para['coeff2'][n * 3] = para['jxy'] / 2
            para['coeff2'][n * 3 + 1] = para['jxy'] / 2
            para['coeff2'][n * 3 + 2] = para['jz']
        for n in range(int(para['positions_h2'].shape[0]/2), para['positions_h2'].shape[0]):
            para['coeff2'][n * 3] = para['jxy'] * para['ratio'] / 2
            para['coeff2'][n * 3 + 1] = para['jxy'] * para['ratio'] / 2
            para['coeff2'][n * 3 + 2] = para['jz'] * para['ratio']
        para['data_exp'] = 'kagome2nColumns%d_ratio%g_j(%g,%g)_h(%g,%g)_chi%d' % \
                           (para['num_columns'], para['ratio'], para['jxy'], para['jz'], para['hx'],
                            para['hz'], para['chi'])
...
def generate_parameters_so_honeycomb():
    ...
    para['chi'] = 12
    para['so_time'] = 20
    if para['state_type'] == 'pure':
        para['tau'] = [1e-1, 1e-2, 1e-3]
        para['beta'] = 10
        para['tol'] = 1e-7
        para['dt_ob'] = 10
        para['ini_way'] = 'random'
    elif para['state_type'] == 'mixed':
        para['tau'] = 1e-2
        para['beta'] = np.arange(0.1, 1.1, 0.1)
        para['ini_way'] = 'id'

    para['d'] = physical_dim_from_spin(para['spin'])
    if para['state_type'] == 'mixed':
        para['d'] *= 2
    para['if_print'] = True
    para['is_debug'] = False
...
def parameter_qes_gs_by_ed(para=None):
    if para is None:
    ...
    para1['hx'] = 0.5
    para1['hz'] = 0
    para1['l_phys'] = 6  # number of physical sites in the bulk

    para1['chi'] = 16  # Virtual bond dimension cut-off
    para1['if_load_bath'] = True
...
def from_index2_to_positions_h2(index2):
    from DMRGalgorithms import sort_positions
    pos_h2 = index2[:, :2]
