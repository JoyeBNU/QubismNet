...
from BasicFunctions import arg_find_array, search_file, load_pr, save_pr
...
def dmrg_infinite_size(para=None, A=None, hamilt=None, env=None):
    ...
    if A is None:
        ...
        A = Minf(para['form'], d, para['chi'],
                 para['d']**para['n_site'], n_site=para['n_site'], spin=para['spin'],
                 is_symme_env=para['is_symme_env'], dmrg_type=para['dmrg_type'],
                 hamilt_index=para['hamilt_index'], env=env)
...
def plot_finite_dmrg(x, A, para, ob):
    ...
    if x == 'eb':  # plot bond energies
        if para['lattice'] == 'chain':
            ...
            if para['bound_cond'] == 'periodic':
                ...
                mp.text(A.chi / 2, ob['eb'][-1] - 0.0002, 'Eb(0, %d) = %g' % (A.chi - 1, ob['eb'][-1]),
    ...
    elif x == 'mag':  # plot magnetization
        mp.subplot(2, 1, 1)
        f1, = mp.plot(range(1, A.chi + 1), ob['mx'], '-ro')
        mp.ylabel(r'$\langle \hat{s}^x \rangle$')
        # mp.legend(handles=[f1, ], labels=[r'$\langle \hat{s}_n^x \rangle$'], loc='best')
        mp.subplot(2, 1, 2)
        f2, = mp.plot(range(1, A.chi + 1), ob['mz'], '--bs')
        ...
        mp.plot(range(1, A.chi), A.ent, '--or')
        mp.xlabel('lattice bond')
